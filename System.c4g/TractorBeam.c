...
private func Timer()
{
  ...
  while(i<iVtxNum)
  {
    ...
    {
		//DrawParticleLine ("LightningSpark", GetVertex(i)-GetX(), GetVertex(i, 1)-GetY(),
		//		GetVertex(i+1)-GetX(), GetVertex(i+1,1)-GetY(), 6, 60, RGB(0,250,250), RGB(200,250,250));
		DrawParticleLine ("NoGravSpark", GetVertex(i)-GetX(), GetVertex(i, 1)-GetY(),
				//GetVertex(i+1)-GetX(), GetVertex(i+1,1)-GetY(),  3, 25, RGB(250,100,100), RGB(250,100,100));
				GetVertex(i+1)-GetX(), GetVertex(i+1,1)-GetY(),  3, 25, RGB(100,100,250), RGB(100,250,100));
    }
    i++;
  }
}
...
private func Remove()
{
  ...
  while(i<GetVertexNum()-1)
  {
	  ...
	  {
			DrawParticleLine ("LightningSpark", GetVertex(i)-GetX(), GetVertex(i, 1)-GetY(),
					GetVertex(i+1)-GetX(), GetVertex(i+1,1)-GetY(), 6, 60, RGB(200,250,250));
			DrawParticleLine ("NoGravSpark", GetVertex(i)-GetX(), GetVertex(i, 1)-GetY(),
					GetVertex(i+1)-GetX(), GetVertex(i+1,1)-GetY(),  3, 25, RGB(250,100,100));
	  }
    i++;
